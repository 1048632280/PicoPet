...
- 常驻内存目标为 release build 下 host + WebView2 子进程 Private Working Set 50-100 MB；总 Working Set 作为 WebView2 共享页诊断值记录，不作为门禁。
...
- 点击穿透关闭后可以再次拖拽桌宠。
- 托盘退出可以完全关闭进程。
- release build 下 host + WebView2 子进程 Private Working Set 保持在 50-100 MB 目标范围内；启动 shell 产生的 `conhost` 如出现在进程树中，不计入应用目标。
